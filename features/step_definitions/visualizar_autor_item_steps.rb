Dado('que existe uma lista de compras chamada {string}') do |nome_lista|
  @lista = List.create!(name: nome_lista, owner: User.first || FactoryBot.create(:user, email: 'owner@test.com'))
end
...
Dado('que há um item {string} sem autor definido') do |item|
  ...
  @unknown_item = item
  unknown_user = FactoryBot.create(:user, email: 'unknown@system.com')
